...
def break_if_python_not_supported():
    if sys.version_info[0:2] < (3,5):
        raise Exception("Wrong python version! Your Python interpreter must be 3.5.0 or above!")
...
if __name__ == '__main__':
    ...
    reporting_relay = heralding.reporting.reporting_relay.ReportingRelay(loop)
    loop.run_until_complete(reporting_relay.create_queue())
    reporting_relay_task = asyncio.ensure_future(reporting_relay.start())
